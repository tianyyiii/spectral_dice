...
class SpectralNeuralDice(object):
  ...
  def __init__(
    ...
    if issubclass(type(self._nu_network), Network):
      self._nu_network.create_variables()
    self._zeta_network = zeta_network
    if issubclass(type(self._zeta_network), Network):
  ...
  def _get_average_value(self, network, env_step, policy):
    if self._solve_for_state_action_ratio:
      ...
      #need to change the dtype of state to be float64 if the env name is reacher
      if self._dataset_spec.observation.dtype == tf.float64 and self._dataset_spec.action.dtype == tf.float32:
        tfagents_step = tfagents_step._replace(observation=tf.cast(tfagents_step.observation, tf.float64))
        env_step = env_step._replace(observation=tf.cast(env_step.observation, tf.float32))
  ...
  def train_step(self, initial_env_step: dataset_lib.EnvStep,
    ...
    with tf.GradientTape(persistent=True) as tape:
      tape.watch(self._nu_network.trainable_variables)
      tape.watch(self._zeta_network.trainable_variables)
      ...
      for var in self._nu_network.trainable_variables:
        print("nu network trainable parameters")
        print(f"Name: {var.name}, Shape: {var.shape}")
      for var in self._zeta_network.trainable_variables:
        print("zeta network trainable parameters")
        print(f"Name: {var.name}, Shape: {var.shape}")

    nu_grads = tape.gradient(nu_loss, self._nu_network.trainable_variables)
    nu_grad_op = self._nu_optimizer.apply_gradients(
        zip(nu_grads, self._nu_network.trainable_variables))

    zeta_grads = tape.gradient(zeta_loss, self._zeta_network.trainable_variables)
    if tf.random.uniform([],minval=0, maxval=1) < 0.002:
      tf.print("nu_grads")
      # tf.print(nu_grads[0][0:100])
      tf.print(tf.norm(nu_grads))
      tf.print("zeta_grads")
      tf.print(tf.norm(zeta_grads))
    zeta_grad_op = self._zeta_optimizer.apply_gradients(
        zip(zeta_grads, self._zeta_network.trainable_variables))
  ...
  def _eval_constraint_and_regs(self, dataset: dataset_lib.OffpolicyDataset,
    ...
    experience = dataset.get_step(10000, num_steps=2)
    env_step = tf.nest.map_structure(lambda t: t[:, 0, ...], experience)
    next_env_step = tf.nest.map_structure(lambda t: t[:, 1, ...], experience)
    #dtype change for reacher env
    if self._dataset_spec.observation.dtype == tf.float64 and self._dataset_spec.action.dtype == tf.float32:
      env_step = env_step._replace(observation=tf.cast(env_step.observation, tf.float32))
      next_env_step = next_env_step._replace(observation=tf.cast(next_env_step.observation, tf.float32))
