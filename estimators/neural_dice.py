...
class NeuralDice(object):
  ...
  def train_loss(self, initial_env_step, env_step, next_env_step, policy):
    ...
    # if tf.equal(tf.random.uniform([], minval=0, maxval=100, dtype=tf.int32), 53):
    #     tf.print(zeta_values.shape)
    #     tf.print(zeta_values)
    #     tf.print("zeta", tf.reduce_mean(tf.abs(zeta_values-1)))
