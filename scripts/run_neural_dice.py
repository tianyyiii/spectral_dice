...
def main(argv):
  ...
  hidden_dims = (64, 64)
  if env_name == "reacher":
    old_spec = dataset.spec.observation
    new_spec = tensor_spec.BoundedTensorSpec(shape=old_spec.shape, dtype=tf.float32, name=old_spec.name, minimum=old_spec.minimum, maximum=old_spec.maximum)
    input_spec = (new_spec, dataset.spec.action)
  else:
