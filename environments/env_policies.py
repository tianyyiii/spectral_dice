...
#######################################policy network for mujoco envs########################
...
def get_sac_mujoco_policy(env_name):
  collect_env = suite_mujoco.load(env_name)
  tf_env = tf_py_environment.TFPyEnvironment(collect_env)
...
def get_env_and_policy(load_dir,
  ...
  elif env_name == "Walker2d-v2" or env_name == "Ant-v2" or env_name == "Hopper-v2":
    env = suites.load_mujoco(env_name)
    env.seed(env_seed)
    tf_env = tf_py_environment.TFPyEnvironment(env)
    sac_policy = get_sac_mujoco_policy(env_name)
    directory = os.path.join(load_dir, env_name, 'train', 'policy')
    policy = load_policy(sac_policy, env_name, directory)
    policy = GaussianPolicy(
        policy, 0.2 - 0.1 * alpha, emit_log_probability=True)
